      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full h-full sm:w-auto sm:h-auto sm:max-w-4xl sm:max-h-[85vh] overflow-hidden sm:rounded-2xl bg-card sm:border sm:border-border/50 shadow-2xl"
          ...
          >
            ...
            <div className="overflow-y-auto h-full sm:max-h-[85vh]">
              <div className="flex flex-col sm:grid sm:grid-cols-1 md:grid-cols-2 gap-0 min-h-full">
                {/* Image Gallery */}
                <div className="relative h-[45vh] sm:h-auto sm:aspect-square md:aspect-auto md:min-h-[400px] overflow-hidden bg-muted/20 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="sm:hidden absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-card via-card/80 to-transparent pointer-events-none" />
                  ...
                  {images.length > 1 && (
                    <>
                      ...
                      {/* Image indicators */}
                      <div className="absolute bottom-20 sm:bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 px-2.5 py-1 rounded-full bg-background/80 backdrop-blur-sm">
                        {images.map((_, idx) => (
                          <button
                            key={idx}
                      ...
                      <div className="sm:hidden absolute bottom-24 left-1/2 -translate-x-1/2 text-[10px] text-muted-foreground bg-background/80 px-2 py-0.5 rounded-full">
                        ← Swipe →
                      </div>
                    </>
                  )}
                ...
                <div className="p-4 space-y-3 flex flex-col flex-1">
                  ...
                  {machine.specs && Object.keys(machine.specs).length > 3 && (
                    <div className="space-y-2 flex-1 min-h-0">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs overflow-y-auto max-h-32 sm:max-h-40 pr-1">
                        {machine.specs.engine && (
                          <div className="flex justify-between py-1 border-b border-border/20 col-span-2">
                  ...
                  {(!machine.specs || Object.keys(machine.specs).length <= 3) && (
                    <div className="grid grid-cols-4 gap-2">
                      ...
                      <div className="p-2.5 rounded-lg bg-muted/30 border border-border/30 text-center">
                        <span className="text-[10px] text-muted-foreground uppercase">{machine.miles ? 'Miles' : 'Hrs'}</span>
                        <p className="font-bold text-sm text-foreground">{machine.miles ? machine.miles.toLocaleString() : machine.hours.toLocaleString()}</p>
                      </div>
                      <div className="p-2.5 rounded-lg bg-muted/30 border border-border/30 text-center">
                        <span className="text-[10px] text-muted-foreground uppercase">Cond.</span>
                  ...
                  <div className="mt-auto pt-2 space-y-3" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        size="lg"
                        className="flex-1 h-12 text-base font-bold bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl transition-colors duration-200"
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="lg"
                        className="h-12 px-4 font-semibold border-primary/50 text-primary hover:bg-primary/10 rounded-xl transition-colors duration-200"
                      >
